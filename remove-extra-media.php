...
<?php wp_nonce_field( self::ID ); ?>
	...
	<p><?php _e( 'Use this tool to process posts for TBD.', 'remove-extra-media' ); ?></p>

	<p><?php _e( 'This processing is not reversible. Backup your database beforehand or be prepared to revert each transformmed post manually.', 'remove-extra-media' ); ?></p>
...
<?php
	...
	public function ajax_process_post() {
		...
		die( json_encode( array( 'success' => sprintf( __( '&quot;<a href="%1$s" target="_blank">%2$s</a>&quot; Post ID %3$s was successfully processed in %4$s seconds.', 'remove-extra-media' ), get_permalink( $this->post_id ), esc_html( get_the_title( $this->post_id ) ), $this->post_id, timer_stop() ) ) ) );
	}
	...
	public static function styles() {
		wp_register_style( 'remove-extra-media', plugins_url( 'remove-extra-media.css', __FILE__ ) );
		wp_enqueue_style( 'remove-extra-media' );
