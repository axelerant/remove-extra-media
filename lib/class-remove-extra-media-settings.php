...
class Remove_Extra_Media_Settings {
	...
	public static function settings() {
		...
		self::$settings['paging'] = array(
			'title' => esc_html__( 'Enable Paging?', 'remove-extra-media' ),
			'desc' => esc_html__( 'For `[rmem_widget_list]`', 'remove-extra-media' ),
			'type' => 'select',
			'choices' => array(
				'' => esc_html__( 'Disable', 'remove-extra-media' ),
				1 => esc_html__( 'Enable', 'remove-extra-media' ),
				'before' => esc_html__( 'Before wps', 'remove-extra-media' ),
				'after' => esc_html__( 'After wps', 'remove-extra-media' ),
			),
			'std' => 1,
			'widget' => 0,
		);
		...
		// Post Type
		$desc        = __( 'URL slug-name for <a href="%1s">wps archive</a> page.', 'remove-extra-media' );
		$has_archive = rmem_get_option( 'has_archive', '' );
		$site_url    = site_url( '/' . $has_archive );

		self::$settings['has_archive'] = array(
			'section' => 'testing',
			'title' => esc_html__( 'Archive Page URL', 'remove-extra-media' ),
...
';
	...
	public static function validate_settings( $input, $options = null, $do_errors = false ) {
		...
		$has_archive  = rmem_get_option( 'has_archive' );
		$rewrite_slug = rmem_get_option( 'rewrite_slug' );
